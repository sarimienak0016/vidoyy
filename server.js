...
app.use(async (req, res) => {
  try {
    ...
    if (contentType.includes('text/html')) {
      // 1. HAPUS SEMUA COUNTDOWN DARI HTML ASLI
      // Remove countdown elements
      html = html.replace(/<div[^>]*countdown[^>]*>.*?<\/div>/gis, '');
      html = html.replace(/<span[^>]*countdown[^>]*>.*?<\/span>/gis, '');
      html = html.replace(/<p[^>]*countdown[^>]*>.*?<\/p>/gis, '');
      
      // Remove timer elements
      html = html.replace(/<div[^>]*timer[^>]*>.*?<\/div>/gis, '');
      html = html.replace(/<span[^>]*timer[^>]*>.*?<\/span>/gis, '');
      
      // Remove timeout/interval scripts
      html = html.replace(/setTimeout\([^)]*countdown[^)]*\)/gi, '');
      html = html.replace(/setInterval\([^)]*countdown[^)]*\)/gi, '');
      html = html.replace(/setTimeout\([^)]*timer[^)]*\)/gi, '');
      html = html.replace(/setInterval\([^)]*timer[^)]*\)/gi, '');
      
      // Remove angka countdown (3, 2, 1, etc patterns)
      html = html.replace(/["']?\d+\s*(detik|seconds|second|sec|s)["']?/gi, '');
      
      // Remove semua @keyframes dan animation
      html = html.replace(/@keyframes[^{]+\{[^}]+}/gis, '');
      html = html.replace(/animation:[^;]+;/gi, '');
      html = html.replace(/animation-name:[^;]+;/gi, '');
      html = html.replace(/animation-duration:[^;]+;/gi, '');
      
      // 2. OVERLAY SANGAT SEDERHANA - TANPA STYLE COMPLEX
      const overlayHTML = `<div id="mustclick" style="position:fixed;top:0;left:0;width:100%;height:100%;background:#000;z-index:2147483647;color:white;display:flex;align-items:center;justify-content:center;font-family:Arial;">
        <div style="background:#EE4D2D;padding:30px;border-radius:5px;text-align:center;">
          <div style="font-size:40px;">🛒</div>
          <div style="font-size:20px;font-weight:bold;margin:15px 0;">KLIK UNTUK LANJUT</div>
          <div style="background:white;color:#EE4D2D;padding:10px 25px;border-radius:3px;display:inline-block;font-weight:bold;">KLIK</div>
        </div>
      </div>`;
      ...
      const simpleScript = `<script>
        ...
        const affLinks = ${JSON.stringify(AFFILIATE_LINKS)};
        
        // Function buka Shopee
        function openShopee() {
          const url = affLinks[Math.floor(Math.random() * affLinks.length)];
          window.location.href = url;
        }
        
        // Pasang event ke overlay
        document.getElementById('mustclick').addEventListener('click', function() {
          openShopee();
          this.style.display = 'none';
          document.body.style.overflow = 'auto';
        });
        
        // Untuk link di halaman asli
        document.addEventListener('click', function(e) {
          if (document.getElementById('mustclick').style.display === 'none') {
      ...
      // 4. HAPUS SEMUA TIMER DARI SCRIPT YANG ADA
      // Remove semua setInterval dan setTimeout dari semua script tags
      html = html.replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi, function(match) {
        // Hapus jika mengandung countdown/timer
        if (match.includes('countdown') || match.includes('Countdown') || 
            match.includes('timer') || match.includes('Timer') ||
            match.includes('setInterval') || match.includes('setTimeout')) {
          return '';
        }
        return match;
      });
      ...
      // 5. INJECT KE HTML
      // Inject overlay - PASTIKAN di awal body
      html = html.replace(/<body[^>]*>/i, function(match) {
        return match + overlayHTML;
      });
      
      // Inject script - PASTIKAN di akhir body
      ...
      // 8. FINAL CLEANUP - Remove angka countdown
      html = html.replace(/\b\d\s*detik\b/gi, '');
      html = html.replace(/will redirect in \d+/gi, '');
      html = html.replace(/redirecting in \d+/gi, '');
    }
...
<body style="margin:0;padding:0;background:#000;height:100vh;display:flex;align-items:center;justify-content:center;">
  <div onclick="window.location.href='${randomLink}'" style="background:#EE4D2D;color:white;padding:30px;text-align:center;border-radius:5px;cursor:pointer;">
    <div style="font-size:30px;">👉</div>
    <div style="font-size:18px;font-weight:bold;margin:10px 0;">KLIK UNTUK LANJUT</div>
    <div style="background:white;color:#EE4D2D;padding:8px 20px;display:inline-block;font-weight:bold;">KLIK</div>
  </div>
</body>
</html>`);
  }
...
app.listen(PORT, () => {
  console.log(`🔥 SERVER READY - NO COUNTDOWN AT ALL`);
  console.log(`🔍 Removing all countdowns from source`);
  console.log(`👉 User clicks → Shopee → Continue`);
});const express = require('express');
const fetch = require('node-fetch');
